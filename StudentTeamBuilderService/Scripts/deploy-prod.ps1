# Deploy StudentTeamBuilderService to Production Environment
# This script publishes and deploys the service to the production environment

Write-Host "[START] Deploying StudentTeamBuilderService to Production..." -ForegroundColor Green
Write-Host ""
...
$serviceName = "StrAppersStudentTeamBuilder"
$publishPath = Join-Path $projectPath "publish-prod"
$deployPath = "C:\Services\StudentTeamBuilderService-Prod"
...
try {
    ...
    if (-not $serviceExists) {
        ...
        sc.exe create $serviceName binPath= "`"$exePath`" Prod" start= auto
        if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host ""
    Write-Host "[OK] Successfully deployed StudentTeamBuilderService to Production!" -ForegroundColor Green
