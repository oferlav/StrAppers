...
- **Google OAuth (StrAppers):** **`src/api/googleAuthStrAppers.js`** calls **`GET {backend}/auth/google/login-url?returnUrl=...`** (return URL should be `{origin}/GoogleCallback`). Google redirects to the **API** at **`GET /auth/google/callback`**; the API exchanges the code, resolves **employer → organization → student** by email (verified Google email), then redirects the browser to **`/GoogleCallback`** with query params (`status`, `userType`, `email`, etc.). **No Google client secret in the SPA** — only **`VITE_STRAPPERS_BACKEND_URL`**. Backend config: **`GoogleAuth:ClientId`**, **`ClientSecret`**, **`RedirectUri`** (must match Google Cloud “Authorized redirect URI”), optional **`FrontendBaseUrl`** (else **`GitHub:FrontendBaseUrl`**, else `https://skill-in.com`). Falls back to **`GoogleWorkspace:OAuth:*`** or **`GoogleWorkspace:ClientId`/`ClientSecret`** if **`GoogleAuth:*`** is unset.
