-- Insert ONLY non-fragmented, non-variant MentorPrompt records for reading prompt from DB.
-- Categories: Platform Context and Vision (1 row), Base Mentor Persona and General Instructions (1 row), Trello Knowledge (2 rows).
-- Excludes: Current Mentor Context, User Message Templates (variants/fragments stay in code or config).
-- Run after InsertPromptCategories_GeneralMentoring_PostgreSQL.sql and AddHasPlaceholdersColumn_MentorPrompt_PostgreSQL.sql (and AddSourceColumn_MentorPrompt_PostgreSQL.sql if present).
-- If re-running: delete existing MentorPrompt rows for these three categories first, or run on a clean table.
