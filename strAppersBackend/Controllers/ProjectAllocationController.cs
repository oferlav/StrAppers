...
namespace strAppersBackend.Controllers
{
    ...
    public class ProjectAllocationController : ControllerBase
    {
        ...
        public ProjectAllocationController(ApplicationDbContext context, ILogger<ProjectAllocationController> logger, SlackService slackService)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<ActionResult<IEnumerable<AvailableProject>>> GetAvailableProjects()
        {
            try
            {
                var availableProjects = await _context.Projects
                    ...
                    .Include(p => p.Status)
                    .Where(p => !p.HasAdmin) // Only projects without admin
        ...
        public async Task<ActionResult<ProjectAllocationResponse>> AllocateStudentToProject(ProjectAllocationRequest request)
        {
            try
            {
                ...
                if (student.ProjectId.HasValue)
                {
                    return BadRequest($"Student {student.FirstName} {student.LastName} is already allocated to project '{student.Project?.Title}'");
                }
                ...
                if (request.IsAdmin && project.HasAdmin)
                {
                ...
                // Update project's HasAdmin status if student is becoming admin (and no conflict)
                if (request.IsAdmin && !adminAllocationFailed)
                {
                    project.HasAdmin = true;
        ...
        public async Task<ActionResult<ProjectAllocationResponse>> DeallocateStudent(int studentId)
        {
            try
            {
                ...
                var project = await _context.Projects
                    .FirstOrDefaultAsync(p => p.Id == student.ProjectId);

                // If student was admin, update project's HasAdmin status
                if (student.IsAdmin && project != null)
                {
                    project.HasAdmin = false;
                    project.UpdatedAt = DateTime.UtcNow;
                }
        ...
        public async Task<ActionResult<ChangeProjectStatusResponse>> ChangeProjectStatusToPlanning(ChangeProjectStatusRequest request)
        {
            try
            {
                ...
                if (!student.ProjectId.HasValue || student.ProjectId != request.ProjectId)
                {
                ...
                {
                    return BadRequest($"Project status can only be changed from 'New' to 'Planning'. Current status is '{project.Status.Name}'");
                }

                // Business Rule: Project must have a student with Backend Developer role
                var hasBackendDeveloper = project.Students
